$inputDirectory = "C:\Users\karce\OneDrive - F5, Inc\Documents\Consulting\FIS_combine\test_batch"
...
# Check if the virtual port is an asterisk (*) and change it to 0
  ...
  $monitorJson = '{"bigip": "/Common/tcp"}'
    ...
    if ($parsedObject.bindings.certs) {
      ...
      # Loop through each certificate entry in the certs array
      foreach ($certEntry in $parsedObject.bindings.certs) {
          # Check if the '-cert' key is present in the certificate entry
          if ($certEntry.'-cert') {
             #Write-Host "Reviewing full path $certEntry"
              # The '-cert' entry contains the full path, so extract just the file name
              $fullCertPath = $certEntry.'-cert'.Trim('"')  # Remove the surrounding quotes
              #Write-Host "Reviewing cert names $fullCertPath"
              if ($fullCertPath -match "/nsconfig/ssl/(.*?)\.cer") {
                #Write-Host "on the last loop" # Regex to capture the file name after the last backslash
                  $certFileName = $matches[1]  # The captured file name
                  #Write-Host "Found cert file name: $certFileName"
                  break # Exit the loop after finding the first certificate file name
              }
          }
      }
  } else {
...
"@                     
          ...
          elseif ($key -eq "-Listenpolicy") {
            ...
            $message | Out-File -FilePath "C:\Users\karce\OneDrive - F5, Inc\Documents\Consulting\FIS_combine\test_batch\listenpolicies.txt" -Append
        } 
        elseif ($key -eq "-backupVServer") {
          ...
          $message | Out-File -FilePath "C:\Users\karce\OneDrive - F5, Inc\Documents\Consulting\FIS_combine\test_batch\backup_servers.txt" -Append
      } 
...
"@
          ...
          $message | Out-File -FilePath "C:\Users\karce\OneDrive - F5, Inc\Documents\Consulting\FIS_combine\lrtc_non-prod_non-ssl_july22\severssl.csv" -Append
